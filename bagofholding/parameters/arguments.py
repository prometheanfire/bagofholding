...
class BagOfHoldingArguments(object):
    def __init__(self, parse=True, *args, **kwargs):
        ...
        for name, options \
            in helpers.extract_options(COMMON_PARAMETERS).iteritems():
                logger.debug("adding option, %s, with args -> (%s) \
                    and kwargs -> (%s)", name, options["args"],
                    options["kwargs"])
